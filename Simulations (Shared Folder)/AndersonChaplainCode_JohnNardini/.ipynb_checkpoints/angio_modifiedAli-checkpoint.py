...
class angio_abm :
	...
	def __init__(self,
		...
		self.segments = []  # Track all vessel segments
		self.sprout_positions = {}  # Track sprout positions
		self.branch_log = []  # To log new branches
		self.join_log = []  # To log joined segments

	def get_new_branches(self):
		# Assuming a branching function adds new sprouts to the model,
		# here we need to log the start and current position of the new sprout

		new_branches = []

		# Iterate through the sprout positions, identify new branches
		for sprout_id, sprout_pos in self.sprout_positions.items():
			# Check if this sprout is a new branch (based on some condition)
			if sprout_id in self.branch_log:
				start_position = self.branch_log[sprout_id]['start']
				end_position = sprout_pos
				new_branches.append({
	...
	# Hypothetical function that creates branches (called elsewhere in simulation)
	def branch(self):
		# Example: suppose branches are created here
		for sprout_id, sprout_pos in self.sprout_positions.items():
			# Some condition for branching (based on your model)
			if np.random.rand() < 0.1:  # Example: random branching probability
				new_sprout_id = len(self.sprout_positions) + 1  # New sprout ID
				self.sprout_positions[new_sprout_id] = sprout_pos  # New sprout starts here
	...
	def get_joined_segments(self):
		joined_segments = []

		# Check for anastomosis (joining of sprouts with other segments)
		for sprout_id, sprout_pos in self.sprout_positions.items():
			for other_sprout_id, other_pos in self.sprout_positions.items():
				if sprout_id != other_sprout_id:
					# Check if these sprouts have joined (based on a proximity condition)
					if np.linalg.norm(np.array(sprout_pos) - np.array(other_pos)) < 0.01:  # Example: joining threshold
						joined_segments.append({
							'parent_segment': sprout_id,
							'child_segment': other_sprout_id
						})
